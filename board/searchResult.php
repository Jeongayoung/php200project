<?php
    ...
    if ($searchKeyword == '' || $searchKeyword == null) { // 검색어 공백 여부 확인
        echo "검색어가 없습니다.";
        exit;
    }

    switch ($searchOption) { // 검색 옵션 올바른 값 여부 확인
        case 'title':
        case 'content':
        case 'tandc':
        case 'torc':
    ...
    $sql .= "JOIN member m ON (b.memberID = m.memberID) "; // 게시물 불러오는 쿼리문

    switch ($searchOption) { // 검색 옵션에 따라 WHERE절 붙여줌
        case 'title':
...
<body>
    ...
    <table>
        ...
        <tbody>
            <?php
    if ($dataCount > 0) {
        for ($i=0; $i<$dataCount; $i++) { // 검색된 게시물 갯수만큼 출력
